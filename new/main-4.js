...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // all collage api get fn
  const allCollagesArr = async (allStatesArr) => {
    try {
      const promises = allStatesArr.map((state) =>
        fetch(`${BASE_API_KEY}/institutions/states/${state.name}`)
          .then((res) => res.json())
          .then((data) => data.data),
      );
      ...
      const merged = results.flat(); // combine all arrays into one
      allCollagesArrValue = merged;
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  };
  ...
  async function getApiByCollageName() {
    collageDataArr.find((collage) => console.log(collage));
  }
